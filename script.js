...
async function fetchWeather(city) {
  ...
  try {
    ...
    alert('Gagal fetch cuaca: ' + err.message);
  } finally {
    showLoader(false);
  }
}
...
async function fetchWeatherByCoords(lat, lon, placeName) {
  ...
  try {
    ...
    alert('Gagal ambil cuaca lokasi: ' + e.message);
  } finally {
    showLoader(false);
  }
}
...
async function fetchForecastByCoords(lat, lon) {
  ...
  try {
    ...
    alert('Gagal ambil forecast: ' + e.message);
  } finally {
    showLoader(false);
  }
}
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        ...
        if (last?.data?.name) updateCurrentWeatherUI(last.data);
      }
    );
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (last?.data?.name) {
    ...
    detectAndLoadLocation(); // pakai lokasi device
  }
});
